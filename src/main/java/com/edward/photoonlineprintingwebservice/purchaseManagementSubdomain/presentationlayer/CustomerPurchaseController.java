...
import com.edward.photoonlineprintingwebservice.purchaseManagementSubdomain.buinsesslayer.PurchaseService;
...
public class CustomerPurchaseController {
    ...
    public ResponseEntity<List<PurchaseResponseModel>> getAllPurchasesForCustomer(@PathVariable String customerId){
        return ResponseEntity.ok().body(purchaseService.getAllPurchasesForCustomers(customerId));
    }

    @GetMapping(value = "/{purchaseId}" ,produces = "application/json")
    public ResponseEntity<PurchaseResponseModel>  getPurchaseForCustomerByPurchaseId(@PathVariable String customerId,
    ...
    public ResponseEntity<PurchaseResponseModel> addPurchaseToCustomer(@RequestBody PurchaseRequestModel purchaseRequestModel
            ,@PathVariable String customerId){

        return ResponseEntity.status(HttpStatus.CREATED).body(purchaseService.addPurchaseToCustomers(purchaseRequestModel, customerId));
    }

    @PutMapping(value = "/{purchaseId}", produces = "application/json", consumes = "application/json")
    public ResponseEntity<PurchaseResponseModel> updateCustomerPurchase(@RequestBody PurchaseRequestModel purchaseRequestModel,
                                                                        @PathVariable String customerId, @PathVariable String purchaseId){
    ...
    public ResponseEntity<Void> removePurchaseFromCustomer(@PathVariable String customerId, @PathVariable String purchaseId){
        purchaseService.removePurchaseFromCustomers(customerId, purchaseId);
